...
const SearchBar = ({ onSubmit, placeholder, onReset, searchQuery }: SearchBarProps) => {
    ...
    return (
        <Form {...form}>
            <form
                className={
                    cn('flex items-center gap-3 justify-between flex-row border-2 rounded-full p-3',
                ...
                <FormField
                    ...
                    render={({ field }) => <FormItem className="flex-1">
                        <FormControl>
                            <Input
                                {...field}
                                className="border-none shadow-none focus-visible:ring-0 text-xl focus:border-none focus:ring-0 rounded-none"
